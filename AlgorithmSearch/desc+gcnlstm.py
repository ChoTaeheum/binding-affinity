...
ligand = np.array(dataset['SMILES'], dtype=np.str)
...
class Regressor(nn.Module):
    def __init__(self):
        ...
        self.lslayers = nn.LSTM(128, 128, num_layers=1, batch_first=True)
    ...
    def forward(self, desc, graph):    # mini-batch, 순서 정확하게 맞음
        ...
        sgraph = dgl.unbatch(graph)
        input_lengths = torch.LongTensor([g.number_of_nodes() for g in sgraph])
        input_lengths, sorted_idx = input_lengths.sort(0, descending=True)
        
        # LSTM_instance input variable
        gc_h = dgl.topk_nodes(graph, 'h', 100)[0]    # batch, top_num_nodes(100), num_feat
        gc_h = gc_h[sorted_idx]
        ls_i = pack_padded_sequence(gc_h, input_lengths.tolist(), batch_first=True)   # lstm-input
        ls_h = torch.zeros(1, 128, 128).cuda()    # (num_layers * num_directions, batch, hidden_size)
        ls_c = torch.zeros(1, 128, 128).cuda()      # (num_layers * num_directions, batch, hidden_size)
        
        ls_o, (ls_h, ls_c) = self.lslayers(ls_i, (ls_h, ls_c))
        ls_h = ls_h[0]
        
        
        cat = tc.cat((ls_h, ds_h), axis=1)
